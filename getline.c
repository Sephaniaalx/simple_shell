...
/**
 * input_buffer - a buffer chained command
 * @info: Pointer to the info_t structure
 * @buf: address of the buffer
 * @len: address of the len variable
 * Return: Returns the bytes that has been read
 */
...
/**
 * read_buffer - function reads a buffer
 * @info: Pointer to the info_t structure
...
/**
 * _getline - function gets the next line input from STDIN
 * @info: Pointer to the info_t structure
 * @ptr: the address of the pointer to the buffer whether preallocated or NULL
 * @length: the size of the preallocated ptr if not it is not NULL
 * Return: Returns s
 */
...
int _getline(info_t *info, char **ptr, size_t *length)
{
	...
	if (s)
		_strncat(new_p, buf + i, h - i);
	else
		_strncpy(new_p, buf + i, h - i + 1);
